...
@testset "Group action testset" begin
    x = [1, 2, 3, 4]
    j = 2
    @test group_action(x, j, 0) == [3, 4, 1, 2]

    @test group_action(x, 0, 1) == [4, 3, 2, 1]

    @test group_action(x, 4, 0) == x

    @test group_action(x, j, 1) == [2, 1, 4, 3]

    @test group_action(x, 1, 1) == [1, 4, 3, 2]
end
...
@testset "Burnside process testset" begin
    n = 10
    reps = 50
    xs, js, rs = burnside_process(n, reps)
    ...
    reps = 50
    k = 4
    xs, js, rs = burnside_process(n, reps, k)
    for (x, j, r) in zip(xs, js, rs)
        @test group_action(x, j, r) == x
    end
end
...
@testset "Stationary distribution testset" begin
    ...
    expected = [16, 2, 4, 2, 4, 8, 4, 8] / 48
    @test p ≈ expected
   
    n = 3
    k = 2
    p = π(n, k)
    expected = [8, 2, 2, 4, 4, 4]/ 24
    @test p ≈ expected
end
